...
describe("SDC functionaly as ERC1155 Token", () => {
  ...
  before(async () => {
    ...
    sdc_object = await SDCER20Factory.deploy(counterparty1.address, counterparty2.address,counterparty1.address, token.address,1000,100);
//    erc20_token = await ERC20Factory.deploy("LiquidityToken","LIT");
  ...
  it("Counterparty1 incepts a trade", async () => {
     const incept_call = await sdc_object.connect(counterparty1).inceptTrade(fpml_data,"blub");
