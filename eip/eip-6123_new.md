...
financial derivative contracts frictionless and scalable in a completely automated way. Counterparty and Settlement Risk ís removed, Delivery-vs-Payment Problem is resolved. 
...
### Applications
The deterministic live cycle functionality is applicable to several use cases.
...
#### Initiation Phase: inceptTrade

A counterparty can initiate a trade by providing the counterparty to trade with, trade data, trade position, payment amount for the trade and initial settlement data. Only registered counteparties are allowed to use that function.
...
This method - either called back from the settlement token direcly or from an eligible address completes the settlement transfer. Might result in a termination or start of the next settlement pase depending on the success flag.
...
Allows eligible parties to confirm a formerly-requested mutual trade termination including termination payment value
