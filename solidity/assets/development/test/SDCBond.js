...
describe("Livecycle Unit-Tests for Smart Bond Contract", () => {
  ...
  before(async () => {
    ...
    const SDCFactory = await ethers.getContractFactory("SmartBondContract");
    token = await ERC20Factory.deploy();

    sdc_bond = await SDCFactory.deploy(issuer.address, bondData, "SEC_ID_123", 1000000000, token.address);
