...
import {marketDataItems} from '../wizard-form-data/market-data-items';
 ...
 */
@Component({
  selector: 'app-wizard-symbols-dialog',
  styleUrls: ['./wizard-symbols-dialog.component.scss'],
  templateUrl: './wizard-symbols-dialog.component.html',
})
export class WizardSymbolsDialogComponent implements AfterViewInit {
  displayedColumns: string[] = ['select', 'symbol', 'curve', 'tenor', 'itemType'];
  ...
  checkboxLabel(row?: JsonMarketDataItem): string {
    ...
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.symbol}`;
  }

  constructor(
    public dialogRef: MatDialogRef<WizardSymbolsDialogComponent>,
